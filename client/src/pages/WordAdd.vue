<script setup lang="ts">
    ...
    const addWord = () => {
      if(newWord.value.length>50){
        ...
        words.value.push({ word: newWord.value, botNotify: newBotNotify.value, selfNotify: newSelfNotify.value })
        newWord.value = ""
        openClearedDialog()
      }
    }
...
<template>
    ...
    <div class="expression">
        ...
        <br>
        <p>
            以下のフォームで登録した単語がtraQ上に投稿された際、DMに通知を送信します。
        </p>
    </div>
    <div>
      <label>
        <input v-model="newWord" type="text" placeholder="登録したい単語をここに入力(50文字以内)" class="inputForm"/>
      </label>
    </div>
    <div class="bot">
        <label>
